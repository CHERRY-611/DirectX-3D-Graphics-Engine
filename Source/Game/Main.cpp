...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
	...
	std::unique_ptr<Game> game = std::make_unique<Game>(L"Skeletal Animation");

    // Phong Skinning
    std::shared_ptr<SkinningVertexShader> skinningVertexShader = std::make_shared<SkinningVertexShader>(L"Shaders/SkinningShaders.fx", "VS", "vs_5_0");
    if (FAILED(game->GetRenderer()->AddVertexShader(L"SkinningVertexShader", skinningVertexShader)))
        return 0;

    std::shared_ptr<PixelShader> skinningPixelShader = std::make_shared<PixelShader>(L"Shaders/SkinningShaders.fx", "PS", "ps_5_0");
